...
set BUILD_DIR=D:\QtSource\Fahrtenbuch\build\Desktop_Qt_6_7_3_MinGW_64_bit-Release
...
cd /d "%DEPLOY%"
"%QT_BIN%\windeployqt.exe" --no-compiler-runtime --no-system-d3d-compiler Fahrtenbuch.exe >nul
echo     OK
echo.
...
echo [6/6] Erstelle Fahrtenbuch_Setup.exe...
cd /d "%INSTALLER%"
if exist Fahrtenbuch_Setup.exe del Fahrtenbuch_Setup.exe
"%IFW_BIN%\binarycreator.exe" --offline-only -c config/config.xml -p packages Fahrtenbuch_Setup.exe
if %ERRORLEVEL% neq 0 (
...
echo  %INSTALLER%\Fahrtenbuch_Setup.exe
echo ============================================================
echo.
pause
